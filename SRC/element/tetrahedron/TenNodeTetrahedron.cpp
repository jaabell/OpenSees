...
TenNodeTetrahedron::setResponse(const char **argv, int argc, OPS_Stream &output)
{
    ...
    else if (strcmp(argv[0], "centroid_stresses") == 0)
    {
        for (int i = 0; i < 1; i++)
        {
            output.tag("Centroid");
        ...
        theResponse =  new ElementResponse(this, 30, Vector(6));

    }
    else if (strcmp(argv[0], "nodal_stresses") == 0)
    {
        for (int i = 0; i < NumGaussPoints; i++)
        {
            output.tag("Node");
        ...
        theResponse =  new ElementResponse(this, 31, Vector(NumStressComponents * NumNodes));
...
{
    static Vector stresses(NumStressComponents);    
    static Vector nodalStresses(NumNodes * NumStressComponents); // Initialize nodal stress values    
    static Vector nodalWeights(NumNodes); // Initialize nodal weights
    ...
    if (responseID == 1)
        ...
        for (int i = 0; i < NumGaussPoints; i++) {
            ...
            const Vector &sigma = materialPointers[i]->getStress();
            // Get material stress response
            stresses(0) += sigma(0) / 4;
            stresses(1) += sigma(1) / 4;
            stresses(2) += sigma(2) / 4;
            stresses(3) += sigma(3) / 4;
            stresses(4) += sigma(4) / 4;
            stresses(5) += sigma(5) / 4;
        }
        return eleInfo.setVector(stresses);
    } else if (responseID == 31) {

        // opserr << "Projecting nodal data for element # " << this->getTag() << endln;

        // // Method 1: Total projection.... 
        // nodalStresses.Zero();
        // nodalWeights.Zero();

        // double gaussPoint[4];
        // double xsj;
        // double shp[4][NumNodes] ;  //shape functions at a gauss point
        
        // computeBasis();


        // for (int i = 0; i < NumGaussPoints; i++) {
        //     const Vector &gaussStresses = materialPointers[i]->getStress();

        //     // Set zeta values for the current Gauss point
        //     gaussPoint[0] = sg[i] ;
        //     gaussPoint[1] = sg[abs(1 - i)] ;
        //     gaussPoint[2] = sg[abs(2 - i)] ;

        //     // opserr << "  GP # " << i 
        //     // << " z0 = " << gaussPoint[0] << " "
        //     // << " z1 = " << gaussPoint[1] << " "
        //     // << " z2 = " << gaussPoint[2] << " " << endln;

        //     // opserr << "   gaussStresses = " << gaussStresses << endln;

        //     // Calculate shape functions for the current Gauss point
        //     shp3d(gaussPoint, xsj, shp, xl);

        //     // opserr << "   xsj = " << xsj << endln;

        //     if (xsj < 0)
        //     {
        //         opserr << "NEGATIVE JACOBIAN!!" << endln;
        //         exit(-1);
        //     }

        //     // Use the shape functions to project gaussStresses to nodal values and accumulate nodal weights
        //     for (int j = 0; j < NumNodes; j++) {
        //         for (int k = 0; k < NumStressComponents; k++) {
        //             nodalStresses(j * NumStressComponents + k) += gaussStresses(k) * shp[3][j] * wg[i] * xsj;
        //         }
        //         nodalWeights(j) += shp[3][j] * wg[i] * xsj;
        //     }
        // }

        // // Normalize the nodal stresses by the nodal weights
        // for (int j = 0; j < NumNodes; j++) {
        //     for (int k = 0; k < NumStressComponents; k++) {
        //         nodalStresses(j * NumStressComponents + k) /= nodalWeights(j);
        //     }
        // }

        //Method 2: Nearest Gauss-point
        Vector gaussStresses0 = materialPointers[0]->getStress();
        Vector gaussStresses1 = materialPointers[1]->getStress();
        Vector gaussStresses2 = materialPointers[2]->getStress();
        Vector gaussStresses3 = materialPointers[3]->getStress();
        // opserr << "Projecting nodal data for element # " << this->getTag() << endln;
        // opserr << "   gaussStresses0("<< materialPointers[0] << ") = " << gaussStresses0 << endln;
        // opserr << "   gaussStresses1("<< materialPointers[1] << ") = " << gaussStresses1 << endln;
        // opserr << "   gaussStresses2("<< materialPointers[2] << ") = " << gaussStresses2 << endln;
        // opserr << "   gaussStresses3("<< materialPointers[3] << ") = " << gaussStresses3 << endln;

        for (int k = 0; k < NumStressComponents; k++) {
            nodalStresses(0+k) = gaussStresses0(k) ;
            nodalStresses(6+k) = gaussStresses1(k) ;
            nodalStresses(12+k) = gaussStresses2(k) ;
            nodalStresses(18+k) = gaussStresses3(k) ;
        }
        // opserr << "   nodalStresses = " << nodalStresses << endln;

        // Store the nodalStresses in the response
        return eleInfo.setVector(nodalStresses);
    }
...
TenNodeTetrahedron::setParameter(const char **argv, int argc, Parameter &param)
{
    ...
    if ((strstr(argv[0], "material") != 0) && (strcmp(argv[0], "materialState") != 0))
    {
        ...
        {
            // opserr << argv[2] << endln ;
            // opserr << pointNum << " " << argv[2] << " " << argc - 2 << " " << param << " " << endln ;
...
TenNodeTetrahedron::shp3d( const double zeta[4], double &xsj, double shp[4][NumNodes], const double xl[3][NumNodes]   )
{
    ...
    if (Jdet != Jdet)
    {
        ...
        opserr << "x4, y4, z4 = " << x4 << " " << y4 << " " << z4 << endln;
        opserr << "x5, y5, z5 = " << x5 << " " << y5 << " " << z5 << endln;
        opserr << "x6, y6, z6 = " << x6 << " " << y6 << " " << z6 << endln;
        opserr << "x7, y7, z7 = " << x7 << " " << y7 << " " << z7 << endln;
        opserr << "x8, y8, z8 = " << x8 << " " << y8 << " " << z8 << endln;
        opserr << "x9, y9, z9 = " << x9 << " " << y9 << " " << z9 << endln;
    }
