...
int ASDConcrete3DMaterial::setParameter(const char** argv, int argc, Parameter& param)
{
	...
	// 4000 - init strain
	if (strcmp(argv[0], "initStrainVol") == 0) {
		double vol0 = (m_init_strain(0) + m_init_strain(1) + m_init_strain(2)) / 3.0;
		param.setValue(vol0);
...
int ASDConcrete3DMaterial::updateParameter(int parameterID, Information& info)
{
	...
	case 4001:
	{
		double vol0 = info.theDouble;
		m_init_strain.Zero();
		m_init_strain(0) = m_init_strain(1) = m_init_strain(2) = vol0;
		return 0;
	}
