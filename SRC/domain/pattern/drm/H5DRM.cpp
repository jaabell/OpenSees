...
bool H5DRM::drm_direct_read(double t)
{
    ...
    // To interpolate displacements velocities (not needed) and accelerations
    auto d_interp = [](double a1, double a2, double d1, double d2, double t, double t1, double t2) -> double {
        return (a1 * pow(t, 3) - 3 * a1 * pow(t, 2) * t2 - a1*t * pow(t1, 2) + 2 * a1*t*t1 * t2 + 2 * a1*t * pow(t2, 2) + a1 * pow(t1, 2) * t2 - 2 * a1*t1 * pow(t2, 2) - a2 * pow(t, 3) + 3 * a2 * pow(t, 2) * t1 - 2 * a2*t * pow(t1, 2) - 2 * a2*t*t1 * t2 + a2*t * pow(t2, 2) + 2 * a2 * pow(t1, 2) * t2 - a2*t1 * pow(t2, 2) + 6 * d1 * t - 6 * d1 * t2 - 6 * d2 * t + 6 * d2 * t1) / (6 * (t1 - t2));
    };

    // auto v_interp = [](double a1, double a2, double d1, double d2, double t, double t1, double t2) -> double {
    //     return (3*a1*pow(t,2) - 6*a1*t*t2 - a1*pow(t1,2) + 2*a1*t1*t2 + 2*a1*pow(t2,2) - 3*a2*pow(t,2) + 6*a2*t*t1 - 2*a2*pow(t1,2) - 2*a2*t1*t2 + a2*pow(t2,2) + 6*d1 - 6*d2) / (6*(t1 - t2));
    // };

    auto a_interp = [](double a1, double a2, double t, double t1, double t2) -> double {
        return (a1 * t - a1 * t2 - a2 * t + a2 * t1) / (t1 - t2);
    };
    ...
    for (int n = 0; n < Nodes.Size(); ++n)
    {
        ...
        DRMDisplacements(3 * local_pos + 0) = d_interp(a1[0], a2[0], d1[0], d2[0], t, t1, t2);
        DRMDisplacements(3 * local_pos + 1) = d_interp(a1[1], a2[1], d1[1], d2[1], t, t1, t2);
        DRMDisplacements(3 * local_pos + 2) = d_interp(a1[2], a2[2], d1[2], d2[2], t, t1, t2);
        ...
        DRMAccelerations(3 * local_pos + 0) = a_interp(a1[0], a2[0], t, t1, t2);
        DRMAccelerations(3 * local_pos + 1) = a_interp(a1[1], a2[1], t, t1, t2);
        DRMAccelerations(3 * local_pos + 2) = a_interp(a1[2], a2[2], t, t1, t2);
    }
