...
// Description: This file contains the implementation for PetscSOEMP
...
PetscSOEMP::PetscSOEMP(PetscSolverMP &theSOESolver, MatType matType, int bs)
  : LinearSOE(theSOESolver, LinSOE_TAGS_PetscSOEMP),
...
int
PetscSOEMP::getNumEqn(void) const
{
  return size;
}

PetscSOEMP::~PetscSOEMP()
{
  cout << "PetscSOEMP::~PetscSOEMP()\n";
  if (theChannels != 0)
  {
    delete [] theChannels;
  }
...
int
PetscSOEMP::setSize(Graph &theGraph)
{
  int result = 0;
  int ierr = 0;
  ...
  if (!init_done)
  {
    ...
    if (FILE *file = fopen(PetscSOEMP_PETSCOPTIONS_FILENAME, "r"))
    {
      fclose(file);
      cout << "        + PetscSOEMP::setSize() [" << processID <<  "] : Found petsc_options.txt file. \n";
      PetscInitialize(0, PETSC_NULL, PetscSOEMP_PETSCOPTIONS_FILENAME, "PetscSOEMP::setSize - Initialize with file");
      PetscPopSignalHandler();
    }
    else
    {
      PetscInitialize(0, PETSC_NULL, (char *)0, "PetscSOEMP::setSize - Initialize");
      PetscPopSignalHandler();
    }
  ...
  PetscSOEMP_DEBUGOUT << "        + PetscSOEMP::setSize() [" << processID <<  "] : Getting local max dof tag\n";
  ...
  PetscSOEMP_DEBUGOUT << "        + PetscSOEMP::setSize() [" << processID <<  "] : Determining global maximum dof tag\n";
  isFactored = 0;
  static ID data(1);
  if (processID != 0)
  {
  ...
  PetscSOEMP_DEBUGOUT << "        + PetscSOEMP::setSize() [" << processID <<  "] : size = "  << size << "\n";


  PetscSOEMP_DEBUGOUT << "        + PetscSOEMP::setSize() [" << processID <<  "] : Allocating memory\n";
  ...
  if (B == 0 || X == 0)
  {
    cerr << "WARNING PetscSOEMP::PetscSOEMP :";
  ...
  if (vectX == NULL)
  {
    std::cerr << "PetscSOEMP::PetscSOEMP : can not allocate memory for vectX \n";
  }
  ...
  if (vectB == NULL)
  {
    std::cerr << "PetscSOEMP::PetscSOEMP : can not allocate memory for vectB \n";
  }
  ...
  PetscSOEMP_DEBUGOUT << "        + PetscSOEMP::setSize() [" << processID <<  "] : Distributing SOE rows amongst processors\n";
  ...
  if (processID >= 0)
  {
    ...
    PetscSOEMP_DEBUGOUT << "        + PetscSOEMP::setSize() - [" << processID <<  "] : Initializing PETSc\n";
    ...
    PetscSOEMP_DEBUGOUT << "        + PetscSOEMP::setSize() - Process " << processID << " owns rows from " << startRow << " to " << endRow << endl;
    ...
    {
      if (d_nnz_global[row] < PetscSOEMP_MIN_DNNZ)
      {
        d_nnz_global[row] =  PetscSOEMP_MIN_DNNZ;
      }
      if (o_nnz_global[row] < PetscSOEMP_MIN_ONNZ)
      {
        o_nnz_global[row] =  PetscSOEMP_MIN_ONNZ;
      }
  ...
  if (solverOK < 0)
  {
    cerr << "WARNING:PetscSOEMP::setSize :";
  ...
  PetscSOEMP_DEBUGOUT << "Process " << processID << " owns rows from " << startRow << " to " << endRow << endl;
...
int
PetscSOEMP::setSize(int MaxDOFtag)
{
  int result = -1;
  cerr << "PetscSOEMP::setSize(int MaxDOFtag) -- DEPRECATED! Should not get called\n";
...
int
PetscSOEMP::addA(const Matrix &m, const ID &id, double fact)
{
  ...
  if (processID >= 0)
  {
    ...
    {
      cerr << "PetscSOEMP::addA() - Matrix and ID not of similar sizes\n";
      return -1;
    }
...
int
PetscSOEMP::addB(const Vector &v, const ID &id, double fact)
{
  if (processID >= 0)
  {
    // check for a quick return
    ...
    if (fact == 1.0)   // do not need to multiply if fact == 1.0
    {
      for (int i = 0; i < idSize; i++)
      {
        ...
        if (pos < size && pos >= 0)
        {
          ...
          if (B[pos] != B[pos])
          {
            cerr << "PetscSOEMP::addB() - rank = " << processID << "- NaN found! At pos=" << pos << endl;
            return -1;
          }
        }
      }
...
int
PetscSOEMP::setB(const Vector &v, double fact)
{
  if (processID >= 0)
  {
    // check for a quick return
...
const Vector &
PetscSOEMP::getX(void)
{
  if (vectX == 0)
  {
    cerr << "FATAL PetscSOEMP::getX - vectX == 0!";
    exit(-1);
  }
...
const Vector &
PetscSOEMP::getB(void)
{
  if (vectB == 0)
  {
    cerr << "FATAL PetscSOEMP::getB - vectB == 0!";
    exit(-1);
  }
...
void
PetscSOEMP::setX(int loc, double value)
{
  if (loc < size && loc >= 0)
  {
    X[loc] = value;
...
void
PetscSOEMP::setX(const Vector &xData)
{
...
int
PetscSOEMP::setSolver(PetscSolverMP &newSolver)
{
  ...
  if (size != 0)
  {
    ...
    if (solverOK < 0)
    {
      cerr << "WARNING:PetscSOEMP::setSolver :";
...
int
PetscSOEMP::sendSelf(int cTag, Channel &theChannel)
{
...
int
PetscSOEMP::recvSelf(int cTag, Channel &theChannel,
...
int
PetscSOEMP::setChannels(int nChannels, Channel **theC)
{
...
int
PetscSOEMP::addM(const Matrix &m, const ID &id, double fact)
{
  ...
  if (idSize != m.noRows() && idSize != m.noCols())
  {
    std::cerr << "PetscSOEMP::addM() - Matrix and ID not of similar sizes\n";
    return -1;
  }
...
int
PetscSOEMP::addK(const Matrix &m, const ID &id, double fact)
{
...
int
PetscSOEMP::setEigenSize(Graph &theGraph)
{
